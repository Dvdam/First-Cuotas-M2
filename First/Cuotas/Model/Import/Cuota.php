...
namespace First\Cuotas\Model\Import;
...
class Cuota extends AbstractEntity
{
    const ENTITY_CODE = 'cuotas_cuota';
    const TABLE = 'cuotas_cuota';
    const ENTITY_ID_COLUMN = 'cuota_id';
    ...
    {
        $cuota = (int) $rowData['cuota'] ?? 0;
        $interes = (int) $rowData['interes'] ?? 0;
    
        if (!$cuota) {
            $this->addRowError('CuotaIsRequired', $rowNum);
        }
    
        if (!$interes) {
            $this->addRowError('InteresIsRequired', $rowNum);
        }
    ...
    private function initMessageTemplates()
    {
        $this->addMessageTemplate(
            'CuotaIsRequired',
            __('Interes should be greater than 0.')
        );
        $this->addMessageTemplate(
            'InteresIsRequired',
            __('Interes should be greater than 0.')
